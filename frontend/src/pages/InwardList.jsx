...
// yyyy-mm-dd in LOCAL time (avoids the UTC day-shift
// you get from toISOString() near midnight).
...
function StatusBadge({ status, statusType, pendingDocuments = 0 }) {
  const styles = {
  ...
  const label =
    statusType === "pending" && pendingDocuments > 0
      ? `${status} (${pendingDocuments} ${pendingDocuments === 1 ? "doc" : "docs"})`
      : status;

  return (
    <span className={`inline-flex whitespace-nowrap rounded-full px-2.5 py-1 text-[11px] font-semibold ${styles[statusType] || styles.complete}`}>
      {label}
    </span>
  );
}
...
// Renders its trigger button INLINE (so it drops into the
// existing filter row with zero layout changes), but portals
// the dropdown panel to document.body and positions it with
// `position: fixed` from the button's own bounding rect, clamped
// to the viewport width so it never gets cut off on small screens.
function DateRangeFilter({ preset, from, to, onPresetSelect, onCustomApply }) {
  ...
  // Close on outside click, and reposition/close on scroll or
  // resize so the panel never drifts away from the button.
  useEffect(() => {
...
export default function Inward() {
  ...
  const listStatus = useSelector(selectInwardListStatus);
  const listError = useSelector(selectInwardListError);
  const history = useSelector(selectInwardHistory);
  const historyStatus = useSelector(selectInwardHistoryStatus);
  ...
  useEffect(() => {
    ...
    dispatch(fetchInwardHistory({ warehouseId: scopedWarehouseId }));
  }, [dispatch, scopedWarehouseId]);

  // Derived/formatted data stays in the component (or could
  // be a reselect selector) - the store just holds raw API data.
  const inwardData = useMemo(() => rawList.map(formatGrn), [rawList]);
  ...
  const filteredData = inwardData.filter((item) => {
    ...
    const searchMatch =
      !searchText ||
      String(item.grn).toLowerCase().includes(searchText) ||
    ...
    let typeMatch = true;
    if (activeFilter === "New Stock") typeMatch = item.type.includes("New Stock");
    if (activeFilter === "Service Stock") typeMatch = item.type.includes("Service Stock");
  ...
  const todaysInward = inwardData.filter((item) => item.date === "Today").length;
  const documentsPending = inwardData.reduce((total, item) => total + (Number(item.pendingDocuments) || 0), 0);
  const completed = inwardData.filter((item) => item.statusType === "complete").length;
  ...
  const handleListDocumentUpload = (event, item) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file || !item?.id) return;
    const allowedTypes = ["application/pdf", "image/jpeg", "image/png", "image/webp"];
    if (!allowedTypes.includes(file.type)) return alert("Only PDF, JPG, PNG and WEBP files are allowed.");
    if (file.size > 10 * 1024 * 1024) return alert("Maximum file size is 10 MB.");
    const existingCount = Array.isArray(item.documents) ? item.documents.length : 0;
    const category = existingCount === 0 ? "Vendor invoice #1" : existingCount === 1 ? "E-way bill #1" : existingCount === 2 ? "Unloading sign-off sheet #1" : `Other document #${existingCount - 2}`;
    dispatch(uploadInwardDocument({ grnId: item.id, file, docCategory: category })).then((result) => {
      if (uploadInwardDocument.fulfilled.match(result)) { alert("Document uploaded successfully."); dispatch(fetchInwardList({ warehouseId: scopedWarehouseId })); dispatch(fetchInwardHistory({ warehouseId: scopedWarehouseId })); }
      else alert(result.payload?.message || "Upload failed.");
    });
  };
    ...
    <div className="min-h-screen bg-[#F1EFE8] overflow-x-hidden">
      <main className="mx-auto max-w-[1200px] px-3 py-4 pb-6 sm:px-6 sm:py-5 lg:px-8">
        <div className="mb-4 flex items-center justify-between sm:mb-5">
          <div>
            <h1 className="text-lg font-bold text-gray-900 sm:text-2xl">Inward entries</h1>
            <p className="mt-0.5 text-[11px] text-gray-500 sm:mt-1 sm:text-sm">View and manage all incoming stock</p>
          </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-2 sm:mb-5 sm:grid-cols-4 sm:gap-3">
          <SummaryCard title="Total entries" value={loading ? "—" : totalEntries} icon={Package} />
          <SummaryCard title="Today's inward" value={loading ? "—" : todaysInward} icon={Clock3} />
            ...
            {filters.map((filter) => (
              <button
                ...
                className={`rounded-lg border px-2 py-1.5 text-[11px] font-medium transition sm:px-3 sm:py-2 sm:text-xs ${
                  activeFilter === filter ? "border-blue-300 bg-blue-50 text-blue-700" : "border-gray-300 bg-white text-gray-700 hover:bg-gray-50"
                }`}
              >
                {filter}
              </button>
        ...
        {loading && (
          <div className="rounded-2xl border border-gray-200 bg-white px-5 py-10 text-center">
            <p className="text-sm text-gray-500">Loading inward entries...</p>
          </div>
        )}
        ...
        {!loading && listError && (
          <div className="rounded-2xl border border-red-200 bg-red-50 p-5">
            ...
            <button
              type="button"
              onClick={() => dispatch(fetchInwardList({ warehouseId: scopedWarehouseId }))}
          ...
          <div className="hidden overflow-hidden rounded-2xl border border-gray-200 bg-white md:block">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[850px] text-left">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500">GRN</th>
                    <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500">Inward type</th>
                    <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500">Company</th>
                    <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500">Supplier / Customer</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredData.map((item) => (
                    <tr key={item.id} onClick={() => openInward(item)} className="cursor-pointer transition hover:bg-gray-50">
                      <td className="px-5 py-4">
                        <p className="text-sm font-semibold text-gray-900">{item.grn}</p>
                      </td>
                      <td className="px-5 py-4">
                        <span className="text-xs text-gray-700">{item.type}</span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="text-xs font-medium text-gray-800">{item.companyName || "-"}</span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="text-xs font-medium text-gray-800">{item.party}</span>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {item.statusType !== "complete" && (
                            <label onClick={(e) => e.stopPropagation()} className="cursor-pointer rounded-lg border border-gray-200 px-2.5 py-1.5 text-[11px] font-medium text-gray-700 hover:bg-gray-50">Upload
                              <input type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png,.webp" onChange={(e) => handleListDocumentUpload(e, item)} />
                            </label>
                          )}
            ...
            {filteredData.length === 0 && (
              <div className="p-10 text-center">
                ...
                <p className="text-sm font-medium text-gray-700">No inward entries found</p>
            ...
            <div className="flex items-center justify-between border-t border-gray-200 px-5 py-3">
              <p className="text-xs text-gray-500">
                Showing 1–{filteredData.length} of {inwardData.length}
              </p>
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-bold text-gray-900">{item.grn}</p>
                          <StatusBadge status={item.status} statusType={item.statusType} pendingDocuments={item.pendingDocuments} />
                        </div>

                        <p className="mt-1 truncate text-xs text-gray-700">
                          {item.companyName || "-"} · {item.type} · {item.party}
                        </p>
                        ...
                        <div className="mt-1.5 flex items-center gap-1.5 text-[10px] text-gray-400">
                          ...
                          {item.serials > 0 && (
                            <>
                              <span>·</span>
                              <span>{item.serials} serials</span>
                            </>
                          )}
                        </div>
                      </div>
...
function SummaryCard({ title, value, icon: Icon, danger = false }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-2.5 sm:p-4">
      <div className="flex items-center justify-between gap-1">
        <p className="truncate text-[10px] font-medium text-gray-500 sm:text-xs">{title}</p>
        <Icon size={15} className={`shrink-0 ${danger ? "text-red-500" : "text-blue-500"}`} />
      </div>
      <p className={`mt-1.5 text-lg font-bold sm:mt-2 sm:text-xl ${danger ? "text-red-600" : "text-gray-900"}`}>{value}</p>
    </div>
  );
}
